import datetime
import moodle_locators2 as locators
...
def create_new_user():
    ...
    for tags in locators.list_of_interests:
        ...
        sleep(0.25)
    # Populating the Optional Section
...
def check_new_credentials():
    if driver.current_url == locators.moodle_users_main_page:
        if driver.find_element(By.ID, 'fgroup_id_realname_grp_label'):
            driver.find_element(By.CSS_SELECTOR, 'input#id_realname')
            print(f'----------User with the name{locators.full_name} is displayed, hence Test Passed')
